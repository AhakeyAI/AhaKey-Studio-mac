...
if [[ "$RELEASE_DISTRIBUTION" == "1" ]]; then
  SIGNING_IDENTITY="$SIGNING_IDENTITY" REQUIRE_DEVELOPER_ID=1 "$SCRIPT_DIR/build.sh"
else
  SIGNING_IDENTITY="$SIGNING_IDENTITY" "$SCRIPT_DIR/build.sh"
fi
...
if [[ -n "$SIGNING_IDENTITY" ]]; then
  ...
  ENTITLEMENTS="$APP_ROOT/AhaKey Studio.xcodeproj/AhaKeyStudio.entitlements"
