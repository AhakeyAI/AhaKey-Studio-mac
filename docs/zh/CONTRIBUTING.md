...
- 新建 Issue 前，先搜索[已有问题](https://github.com/ZephyrKeXiner/AhaKey-Studio/issues)。
- 反馈 Bug 时，请提供操作系统、应用版本、相关键盘固件版本、复现步骤、预期行为和实际行为。附上相关日志或截图，并移除凭证及个人信息。
- 建议功能时，请说明希望改善的使用场景。较大的改动建议先通过 Issue 讨论，再开始实现。

## 提交 Pull Request

1. Fork [ZephyrKeXiner/AhaKey-Studio](https://github.com/ZephyrKeXiner/AhaKey-Studio)，克隆自己的 Fork，并为改动创建分支。
2. 阅读[仓库结构](../repo-layout.md)、[架构说明](architecture.md)和[构建说明](../installation.md)。涉及设备交互时，也请阅读 [BLE 协议](../ble-protocol.md)。
3. 保持改动聚焦，遵循周围代码的风格，并更新相关文档；请同步维护[多语言文档索引](../README.md)中列出的中英日版本，包括项目 README。
4. 完成下表中的相关检查。涉及硬件或界面时，请说明手动验证内容，并注明无法测试的部分。
5. 向官方仓库发起 [Pull Request](https://github.com/ZephyrKeXiner/AhaKey-Studio/pulls)，说明解决的问题、具体改动、关联 Issue 和验证结果。涉及界面变化时，请附上截图。
...
| 改动范围 | 执行目录 | 检查方式 |
|---|---|---|
| macOS 应用 / 后台 Agent | 仓库根目录 | 打开 `AhaKey Studio.xcodeproj`，选择应用 Scheme，使用 `⌘B` / `⌘U` |
| TypeScript SDK | `sdks/typescript/` | `npm ci`，然后运行 `npm run typecheck` 和 `npm test` |
| 文档 | 修改的文件 | 按[多语言文档索引](../README.md)检查链接、Markdown 展示和多语言一致性 |
